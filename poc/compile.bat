...
my $content = read_file ( 'merged.pack.js.map' );
...
my $src = read_file ( 'merged.join.js' );
my $org = read_file ( 'merged.pack.js' );
...
foreach my $line (@{$srcmap->{'lines'}})
{
	...
	foreach my $entry (@{$line})
	{
		...
		if (scalar(@{$entry}) >= 4)
		{
			...
			my $excerpt = substr($srcs[$row], $col - 10, 10) .
				'>>' . substr($srcs[$row], $col, 10);
...
write_file ( 'test.pack.html', join("\n", @lines) );
