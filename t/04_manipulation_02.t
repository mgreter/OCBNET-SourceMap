...
is    (scalar(@{$smap->{'mappings'}}),           3,       'mappings array has correct line number');

is    (scalar(@{$smap->{'mappings'}->[0]}),      3,       'mappings line 0 array has correct group number');
...
is    (scalar(@{$smap->{'mappings'}->[1]}),      3,       'mappings line 1 array has correct group number');
is    ($smap->{'mappings'}->[1]->[0]->[4],       0,       'name 4 is correct'); # X
is    ($smap->{'mappings'}->[1]->[0]->[1],       0,       'source 4 is correct'); # 0
is    ($smap->{'mappings'}->[1]->[0]->[0],       1,       'offset 4 is correct'); # X
is    ($smap->{'mappings'}->[1]->[1]->[4],       1,       'name 5 is correct'); # Y
is    ($smap->{'mappings'}->[1]->[1]->[1],       0,       'source 5 is correct'); # 0
is    ($smap->{'mappings'}->[1]->[1]->[0],       3,       'offset 5 is correct'); # Y
is    ($smap->{'mappings'}->[1]->[2]->[4],       2,       'name 6 is correct'); # Z
is    ($smap->{'mappings'}->[1]->[2]->[1],       0,       'source 6 is correct'); # 0
is    ($smap->{'mappings'}->[1]->[2]->[0],       5,       'offset 6 is correct'); # Z

####### remove operation ##########
$smap->mixin([0, 5], [1, 0], undef);
substr($data, 5, 1, '');
substr($data, 5, 1, '');
# die '[' . $data . ']';
# => "A B C X Y Z"

is    (scalar(@{$smap->{'mappings'}}),           2,       '2: mappings array has correct line number');

is    (scalar(@{$smap->{'mappings'}->[0]}),      6,       '2: mappings line array has correct group number');
...
is    ($smap->{'mappings'}->[0]->[3]->[0],       6,       '2: offset 4 is correct'); # X
is    ($smap->{'mappings'}->[0]->[4]->[0],       8,       '2: offset 5 is correct'); # Y
is    ($smap->{'mappings'}->[0]->[5]->[0],       10,      '2: offset 6 is correct'); # Z
